...
Extend the merged ash playground so the same sandbox image contains CPython,
and the same host PTY can run both a one-shot Python command and an interactive
Python REPL.
...
The image recipe will stage CPython's packaged files under `/usr/local`,
provide `/usr/local/bin/python3`, and keep `PYTHONHOME=/usr/local` in the
existing ash environment. The image revision and SHA-256 will be pinned in the
playground only after the composed image is built and exercised.
...
entry falls back to the network; a hash mismatch is discarded and reported.
Deno tests will use injected fetch/cache implementations so the behavior is
...
and assert `1048576`, then start `python3`, submit an expression, and assert
the result before stopping the session. These tests must fail when the image
does not contain Python and must run in CI with materialized pinned artifacts;
they cannot silently skip when Task 5 artifacts are configured.
