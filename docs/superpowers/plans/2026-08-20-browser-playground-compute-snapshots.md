...
- [ ] **Step 3: Implement the CPython 3.14.0 verifier.** Invoke only the CI-provisioned interpreter and assert `sys.implementation.name == "cpython"`, `sys.version_info == (3, 14, 0)`, and x86_64 architecture.
- [ ] **Step 4: Implement deterministic staging.** Materialize with hash checking and no binary extensions, remove excluded packages, normalize the output tree, emit the package input, and compute the normalized-tree digest.
- [ ] **Step 5: Implement the canonical USTAR serializer.** Use the spec's 512-byte USTAR headers, rightmost fitting path split, fixed octal/checksum fields, zero metadata, UTF-8 validation, and symlink rejection rules.
- [ ] **Step 6: Run the materialization tests from a clean output directory and commit the lock, materializer, serializer, and concrete digests.**
...
- [ ] **Step 2: Wire NumPy staging and yurt-jupyter package staging into the image recipe.**
- [ ] **Step 3: Run the existing NumPy smoke plus import checks for `ipykernel` and `jupyter_client`.**
...
- [ ] **Step 5: Commit CI/pin changes and verify the hosted workflow exercises integration tests rather than skipping them.**
...
**Interfaces:**
- `interface SnapshotHost { preflight(bytes: Uint8Array): Promise<RestorePlan>; capture(): Promise<Uint8Array>; commitRestore(plan: RestorePlan): Promise<SessionTransportSet>; rollbackRestore(plan: RestorePlan): Promise<void>; }`
- `saveSnapshot(host: SnapshotHost): Promise<Blob>`
- `restoreSnapshot(host: SnapshotHost, file: File): Promise<void>`
...
- [ ] **Step 4: Implement restore without detaching current pumps before commit; retain the old transport set as rollback state.**
